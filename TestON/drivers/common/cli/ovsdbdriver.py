...
class OvsdbDriver( CLI ):
    ...
    def setHostportGateway(self, hostname="host1", gateway="192.168.33.254" ):
        command = "sudo ip netns exec " + str(hostname) +" route add default gw " + str(gateway) 
        try:
            handle = self.execute(
                cmd=command,
                timeout=10)
            if re.search( "Error", handle ):
                main.log.error( "Error in set gateway:" + str( gateway ) + " on host " + str( hostname ) )
                main.log.error( handle )
                return main.FALSE
            else:
                main.log.info( "Set host gateway:" + str( gateway ) + " on host " + str( hostname ) + " sucess" )
                return main.TRUE
        except pexpect.EOF:
            main.log.error(self.name + ": EOF exception found")
            main.log.error(self.name + ":     " + self.handle.before)
